...
//	import mx.collections.ArrayCollection;<--------------------------------------------Collections Required??
	...
	public double dispersion;				// dispersion of pancake upon impact, 2 * semimajor axis of pancaked impactor
	public double delta;				// angle of the distance entered, measured from center of the earth
	...
	public double melt_coeff =8.9 * Math.pow(10,-21);		// coefficient for melt volume calc
	...
	public double lEarth =5.86 * Math.pow(10,33);		// angular momen. of earth in (kg m^3)/sec
	public double lratio;				// ratio of proj ang. momen, to $lEarth
	public double pEarth =1.794 * Math.pow(10,32);		// lin. momen of earth in (kg * m) / sec
	 ...
	 * @param ctx The Android contexts useful for getting strings.
	 */
	//==================================================================
	...
	{	
		...
		/*if (impactor.imDist < impactor.crDiam){
			impactor.imDist = impactor.crDiam /100;
			loadParameters();
			main_calculation();
		}*/
		sentData();
	...
	public void  loadParameters()
	{
		...
		impactor.tgType = DataProvider.getCbTgDens();
	}//end load parameters===============================================
	...
	public void  sentData()
	{
		...
		InArray.put(getString(R.string.lbImM), standform(impactor.crMass)+" kg");
		InArray.put(getString(R.string.lbPjVel),  nbFormat.format(impactor.pjVel)+" km/s");
		InArray.put(getString(R.string.lbPjAng),  nbFormat.format(impactor.pjAngle)+"º");
		InArray.put(getString(R.string.lbPjDens),  nbFormat.format(impactor.pjDens)+" kg/m³");
		InArray.put(getString(R.string.lbTgDens),   nbFormat.format(impactor.tgDens)+" kg/m³");
		...
		if (impactor.crDepth > 1)
			Outarray.put(getString(R.string.lbCrDepth), nbFormat.format(impactor.crDepth)+" m" );
			
		if (impactor.crDiam > 1)
			Outarray.put(getString(R.string.lbCrDiam), nbFormat.format(impactor.crDiam)+" m" );
		
		if (impactor.ejThickness > 0.0001  )
			Outarray.put(getString(R.string.lbCrThickness),  nbFormat2.format(impactor.ejThickness)+" m");
			
		
		if (impactor.abAltBurst !=0)
			Outarray.put(getString(R.string.lbAbBurst),  nbFormat.format(impactor.abAltBurst)+" m");
		
		if (impactor.abAltBreak !=0)
			Outarray.put(getString(R.string.lbAbBreak),  nbFormat.format(impactor.abAltBreak)+" m");
		
		if (impactor.abWindvel > 0.5 )
			Outarray.put(getString(R.string.lbAbVel),  nbFormat.format(impactor.abWindvel)+" m/s");
		
		Outarray.put(getString(R.string.lbAbRichter),  nbFormat.format(impactor.smRichter) +" m/s");
		
		if (impactor.abAmpl > 0  )
			Outarray.put(getString(R.string.lbAbAmpl),  nbFormat.format(impactor.abAmpl)+" dB");
	
		
		HashMap<String,String> firearray = new HashMap<String,String>();
		firearray.put(getString(R.string.lbFbRad), nbFormat2.format(impactor.fbRadius)+" km" );
		firearray.put(getString(R.string.lbFbDuration),  nbFormat3.format(impactor.fbDuration)+" s" );
		firearray.put(getString(R.string.lbFbPeaktime), standform(impactor.fbPeaktime)+" s");
		
	
		
		if (impactor.fbExposure != 0)
			firearray.put(getString(R.string.lbFbExp), standform(impactor.fbExposure)+" J/m²");
		...
		energyarray.put(getString(R.string.lbKE), standform(impactor.pjEnergy)+" J");
		energyarray.put(getString(R.string.lbImE), standform(impactor.imEnergy)+" J");
		energyarray.put(getString(R.string.lbFreq), nbFormat.format(impactor.imFreq)+" yrs");
		...
		if(iFactor >= 1)
		{
				impactor.imDesc += getString(R.string.projectile1)+nbFormat.format(impactor.imVel)+" km/s \n\n";
				impactor.imDesc += getString(R.string.projectile2)+standform(impactor.imEnergy)+" J";
		}//end if
		else
		{
				if(impactor.abAltBurst > 0)
				{
					impactor.imDesc += getString(R.string.projectile3)+nbFormat.format(impactor.abAltBurst)+" m\n\n";
					impactor.imDesc += getString(R.string.projectile4)+nbFormat.format(impactor.imVel)+" km/s\n\n";
					impactor.imDesc += getString(R.string.projectile5)+standform(impactor.imEnergy)+" J \n\n";
					impactor.imDesc += getString(R.string.projectile6);	
				}//end if
				else
				{
					impactor.imDesc += getString(R.string.projectile7)+nbFormat2.format(impactor.imVel)+" km/s";
					//impactor.imDesc += "The broken projectile fragments strike the ground in an ellipse of dimension <b>%g km by %g km</b>", FormatSigFigs(dispersion/(1000 * sin(theta * pi /180)), sigfigs), FormatSigFigs(dispersion / 1000, sigfigs));
				}//end else
		}//end else
			
		Log.i(LOGTAG,"Impact vel"+impactor.imVel);
		/*}else{
			impactor.imDesc = "The impactor was cometary and it is likely that no solid fragments hit the earth.";
		}*/
		...
		//lbDamage.text = dsLanguage.lastResult.damage1+nbFormat.format(impactor.imDist)+dsLanguage.lastResult.damage2;
	...
	public void calc_energy ()
	{
		...
		energy0_megatons = impactor.pjEnergy / (4.186 * Math.pow(10,15));		////// joules to megatons conversion
		...
		angmom = impactor.crMass * (impactor.imVel * 1000) * Math.cos(impactor.pjAngle * Math.PI / 180) * R_earth;

		if(impactor.pjVel > (0.25 * 3 * Math.pow(10,5)))
		{		// relativistic effects, multiply energy by 1/sqrt(1 - v^2/c^2)
			beta = 1/ Math.sqrt(1 - Math.pow(impactor.pjVel,2) / 9 * Math.pow(10,10));
		...
		if(impactor.abAltBurst > 0)
		{
			impactor.imEnergy = 0.5 * impactor.crMass * (Math.pow(impactor.pjVel * 1000,2) - Math.pow(impactor.imVel * 1000,2));
			impactor.imMegaton = impactor.imEnergy / (4.186 * Math.pow(10,15));		////// joules to megatons conversion
		}//end if
		else
		{
			impactor.abAltBurst = 0;
			impactor.imEnergy = 0.5 * impactor.crMass * Math.pow(impactor.imVel * 1000,2);
			impactor.imMegaton = impactor.imEnergy / (4.186 * Math.pow(10,15));		////// joules to megatons conversion
		}//end else
		...
		mwater = (Math.PI * Math.pow(impactor.pjDiam,2) / 4) * (impactor.tgDepth / Math.sin(impactor.pjAngle * Math.PI / 180)) * 1000;	
		vseafloor = impactor.imVel * Math.exp(-(3 * 1000 * 0.877 * impactor.tgDepth) / (2 * impactor.pjDens * impactor.pjDiam * Math.sin(impactor.pjAngle * Math.PI / 180)));
		...
		impactor.imFreq = 110 * Math.pow(energy0_megatons * Math.pow(10,megaton_power),0.77);
		
		/*if(impactor.imFreq > 1000){
			freq_p = Math.log(impactor.imFreq)/Math.LN10;
			freq_p = int(freq_p);
	...
	public void atmospheric_entry()
	{
		...
		double vSurface;		//velocity of the impactor at surface in m/s if greater than terminal velocity
		double altitude1;
		double omega;
		double vBU;
		double tmp;
		...
		yield = Math.pow(10,(2.107 + 0.0624 * Math.pow(impactor.pjDens,0.5))); //yield strength of projectile in Pa
	        
		av = 3 * rhoSurface * dragC * scaleHeight / (2 * impactor.pjDens * impactor.pjDiam * Math.sin(impactor.pjAngle * Math.PI / 180)); //velocity decrement factor

		rStrength = yield / Math.pow(rhoSurface * impactor.pjVel * 1000 ,2); //strength ratio
		...
		else // projectile does not land intact
		{
			...
			omega = 1.308 - 0.314 * iFactor - 1.303 * Math.pow((1 - iFactor),0.5);
			impactor.abAltBreak = altitude1 - omega * scaleHeight;
			vBU = impactor.pjVel * 1000 * Math.exp(- av * Math.exp(- impactor.abAltBreak/scaleHeight));		// m/s
			
			vFac = 1.5 * Math.pow((dragC * rhoSurface / (2 * impactor.pjDens)),0.5) * Math.exp(- impactor.abAltBreak / (2 * scaleHeight));
			lDisper = impactor.pjDiam * Math.sin(impactor.pjAngle * Math.PI / 180) * Math.pow((impactor.pjDens / (2 * dragC * rhoSurface)),0.5) * Math.exp(impactor.abAltBreak / (2 * scaleHeight)); 

			alpha2 = Math.pow((Math.pow(fp,2) - 1),0.5);
			altitudePen = 2 * scaleHeight * Math.log(1 + alpha2 * lDisper /(2 * scaleHeight));
			impactor.abAltBurst = impactor.abAltBreak - altitudePen;
			trace ("Projectile does not land");
			
			if(impactor.abAltBurst > 0)
			{			// impactor bursts in atmosphere
				expfac = 1/24 * alpha2 *(24 + 8 * Math.pow(alpha2,2) + 6 * alpha2 * lDisper / scaleHeight + 3 * Math.pow(alpha2,3) * lDisper / scaleHeight);
				impactor.imVel = vBU * Math.exp(- expfac * vFac);
			}//end if
			else
			{
				altitudeScale = scaleHeight / lDisper;
				integral = Math.pow(altitudeScale,3) / 3 * (3 * (4 + 1/Math.pow(altitudeScale,2)) * Math.exp(impactor.abAltBreak / scaleHeight) + 6 * Math.exp(2 * impactor.abAltBreak / scaleHeight) - 16 * Math.exp(3 * impactor.abAltBreak / (2 * scaleHeight)) - 3 / Math.pow(altitudeScale,2) - 2);
				impactor.imVel = vBU * Math.exp(- vFac * integral);
				dispersion = impactor.pjDiam * Math.pow((1 + 4 * Math.pow(altitudeScale,2) * Math.pow((Math.exp(impactor.abAltBreak / (2 * scaleHeight)) - 1),2)),0.5);
	...
	public void find_crater()
	{
		...
		anglefac = Math.pow((Math.sin (impactor.pjAngle * Math.PI / 180)),(1/3));
		...
		if(impactor.tgDepth != 0)
		{		// calculate crater in water using Cd = 1.88 and beta = 0.22
		
			wdiameter = 1.88 * (Math.pow((impactor.crMass / impactor.tgDens),1/3)) * Math.pow(( (1.61*G*impactor.pjDiam)/Math.pow((impactor.imVel*1000),2)),(- 0.22));
			wdiameter *= anglefac;
			
			impactor.tgDens = 2700;	// change target density for seafloor crater calculation
		}//end else
		
			// vseafloor == surface velocity if there is no water
		impactor.crTsDiam = Cd * (Math.pow((impactor.crMass / impactor.tgDens),0.333)) * Math.pow(( (1.61*G*impactor.pjDiam)/Math.pow((vseafloor*1000),2)),(- beta));
		impactor.crTsDiam *= anglefac;
		
		if(dispersion >= impactor.crTsDiam)		// if crater field is formed, compute crater dimensions assuming 
			impactor.crTsDiam /= 2;			// impact of largest fragment (with diameter = 1/2 initial diameter)
		
		impactor.crTsDepth = impactor.crTsDiam / 2.828;

		if(impactor.crTsDiam*1.25 >= 3200)
		...
		{                                  
		  ...
		  rimHeightf = 0.07 * Math.pow(impactor.crTsDiam,4) / Math.pow(impactor.crDiam,3);

		  //Thickness of breccia lens in m
		  impactor.crBrecThick = 2.8 * vbreccia * ((impactor.crTsDepth + rimHeightf) / (impactor.crTsDepth * Math.pow(impactor.crDiam,2)));
		
		  //Final crater depth (in m) = transient crater depth + final rim height - breccia thickness
		  impactor.crDepth = impactor.crTsDepth + rimHeightf - impactor.crBrecThick;

		}//end else
		
		impactor.crVol = (Math.PI / 24) * Math.pow((impactor.crTsDiam/1000),3);
		vratio = impactor.crVol / vEarth;
		
		if(impactor.imVel >= 12)
		{
			impactor.crVolMelt = melt_coeff * (energy_seafloor) * Math.sin(impactor.pjAngle * Math.PI / 180);		// energy_seafloor = impactor.imEnergy if there is no water layer
	...
	public void find_ejecta()
	{
		...
		double e = -Math.pow((0.5 * Math.pow((X - 1),2) + 0.5),0.5); // eccentricity of eliptical path of the ejecta
		double a = (X * R_earth* 1000) / (2 * (1 - Math.pow(e,2)));	// semi major axis of elliptical path

		double part1 = Math.pow(a,1.5) / Math.pow((G * Math.pow(R_earth* 1000,2)), 0.5);
		double term1 = 2* Math.atan( Math.pow(((1 - e)/(1 + e)),0.5) * Math.tan (phi / 2));
		double term2 = e * Math.pow((1 - Math.pow(e,2)),0.5) * Math.sin (phi)/ (1 + e * Math.cos(phi));
	
	
		impactor.ejFalltime = 2 * part1 * (term1 - term2);

		impactor.ejThickness = Math.pow(impactor.crTsDiam,4)/(112 * Math.pow(impactor.imDist * 1000,3));
		Log.i(LOGTAG,"Ejecta thickness "+impactor.ejThickness);

		////// compute mean fragment size
		double a2 = 2.65;
		double half_diameter = (impactor.crDiam/1000)/2;		// half of final crater diameter in km
		double dc = 2400* Math.pow(half_diameter,-1.62);

		impactor.ejFragsize = dc * Math.pow((half_diameter/impactor.imDist), a2);
	...
	public void find_thermal()
	{
		...
		impactor.fbRadius = 2* Math.pow(10,-6)* Math.pow(impactor.imEnergy,1/3);				//// impactor.fbRadius is in km
		sigma = 5.67 * Math.pow(10,-8);						//// Stephan-Boltzmann constant

		impactor.fbExposure = (eta * impactor.imEnergy)/(2 * Math.PI * Math.pow(impactor.imDist* 1000,2));

		h = (1 - Math.cos(delta * Math.PI/180))* R_earth;	// h is in km, R_earthis in km	
		...
		impactor.fbPeaktime = (eta * impactor.imEnergy)/(2 * Math.PI * Math.pow(impactor.fbRadius*1000,2) * sigma * Math.pow(T_star,4));
	...
	{	
		...
		if(impactor.imDist >= 700)
		{
			...
			Ax = 20 * Math.pow(10,(impactor.smRichter - 1.66 * (Math.log (delta) / Math.log(10)) - 3.3));
			Ax /= 1000;  //factor for determining "effective impactor.smRichter" at given impactor.imDist

		}//end if
		else if(impactor.imDist >= 60)
			Ax = Math.pow(10,(impactor.smRichter - (0.0048*impactor.imDist + 2.5644)));
		else
			Ax = Math.pow(10,(impactor.smRichter - (0.00238*impactor.imDist + 1.3342)));
	...
	public void air_blast()
	{
		...
		int r_cross0 = 290;	// radius at which relationship between overpressure and distance changes (for surface burst)
		...
		double d_smooth;
		double p_machT;
		double p_0 = 0;//=0 added by AS to stop uninitialise var error when trans from action script.
		double expFactor = 0;//=0 added by AS to stop uninitialise var error when trans from action script.
		double p_regT = 0;//=0 added by AS to stop uninitialise var error when trans from action script.

		energy_ktons = 1000 * impactor.imMegaton; //energy in kilotons

		slantRange = Math.pow(Math.pow(impactor.imDist,2) + Math.pow((impactor.abAltBurst/1000),2),0.5);		// for air burst, distance is slant range from explosion
		impactor.abShockTime = (slantRange * 1000)/vsound;	// distance in meters divided by velocity of sound in m/s
	  ...
	  //radius at which relationship between overpressure and distance changes
		...
		if(d_scale >= (r_mach + d_smooth))
			impactor.abOpressure = ((r_cross * op_cross) / 4) * (1 / d_scale) * (1 + 3*Math.pow((r_cross / d_scale), 1.3));
		else if(d_scale <= (r_mach - d_smooth))
			impactor.abOpressure = p_0 * Math.exp(expFactor * d_scale);
		else
			impactor.abOpressure = p_regT - (d_scale - r_mach + d_smooth) * 0.5 * (p_regT - p_machT)/d_smooth;
		

		impactor.abWindvel = ((5 * impactor.abOpressure) / (7 * PO)) * (vsound / Math.pow((1 + (6 * impactor.abOpressure) / (7 * PO)),0.5));
		...
		if(impactor.abOpressure > 0)
			impactor.abAmpl = 20 * (Math.log (impactor.abOpressure) / Math.log(10));
		else
			impactor.abAmpl = 0;
	...
	public void print_atmosphere()
	{
		...
		double en = 0.5 * impactor.pjMass * (Math.pow(impactor.pjVel * 1000,2) - Math.pow(impactor.imVel * 1000,2));
		double en_power;
		double ens_power;
		double en_mton;
		double enmton_power;
	...
	private String standform(double a)
	{
		...
		return nbFormat2.format(mantissa)+" x 10^"+exponent;
	} //==========================================================================
